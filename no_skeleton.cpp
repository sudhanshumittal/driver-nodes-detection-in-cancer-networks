...
void getCorrelation(std::map< string, float > &dataMap, char* cor_file);
...
{
	if(argc < 5){
		printError("usage ./a.out correlation_file ppi_file_name edge_label_output_file vertex_labels_file\n");
	}   
...
void getCorrelation(std::map < string, float> &dataMap, char* correlation_file){
	ifstream fp;
...
void createGraph(graph &g, char* ppi_file, char* edgeLabels, std::map<string, float> & dataMap){
	...
	//while(fp.good()){
	for( map<string, float>::iterator i= dataMap.begin(); i != dataMap.end(); i++){ 	
		string protein1(""), protein2("");
		//read edges, nodes
		string pro1_pro2 = i->first;
		if (pro1_pro2[0] == '|' || pro1_pro2[pro1_pro2.size()-1]== '|' )
			continue;
		float cor = i->second;
		if ( cor <PEARSON_FACTOR && cor> -PEARSON_FACTOR ) 
			continue;
		int j=0;
		for(; pro1_pro2[j] != '|'; j++)
			protein1+=pro1_pro2[j];
		for( j = j+1; j<pro1_pro2.size(); j++)
			protein2+=pro1_pro2[j];
